...
**Langue de l'interface (backlog § BL).** Français (référence), anglais, espagnol, allemand, italien. C'est un réglage **du foyer** (`user_parametres`, clé `langue`, défaut `fr` ; `PATCH /api/auth/foyer/langue`, propriétaire seul ; exposé par `/api/auth/me`), que ses membres et invités suivent. Avant connexion, la langue est celle de l'appareil : dernier choix mémorisé, sinon la première langue du navigateur que l'application propose, sinon le français ; le premier compte crée son foyer dans cette langue (`POST /api/auth/register`, champ `langue`). Nombres, montants et dates suivent la langue (formats `Intl` de la langue) ; la devise reste l'euro. Traduction livrée par lots : la coquille (navigation, barre de contrôles, connexion, assistant) d'abord, les écrans ensuite — une clé encore absente d'une traduction retombe sur le français, jamais sur un blanc.
