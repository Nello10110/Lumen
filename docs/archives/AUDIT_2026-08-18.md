# Archive — Audit technique et fonctionnel du 18-19/08/2026

> **Document archivé, non maintenu.** Ce fichier est l'ancien `docs/BACKLOG.md`, conservé tel
> quel comme trace de l'audit qui a servi de socle au chantier de refonte du 18-19/08/2026 (55
> points relevés, 53 traités et vérifiés, 2 hors périmètre assumés). Repris le 19/08/2026 dans le
> nouveau `docs/BACKLOG.md`, désormais tourné vers la suite (évolutions fonctionnelles) plutôt que
> vers cet audit, entièrement clos. Son bilan de clôture est repris dans l'annexe A de
> `docs/BACKLOG.md`. Le contenu ci-dessous n'est plus mis à jour ; il fait foi pour comprendre *pourquoi*
> une décision a été prise à l'époque, pas pour l'état actuel du produit.
...
des hypothèses de bug sur les données réelles de la base).
...
Les resoustraire les compte une seconde fois. Impact mesuré sur la base réelle : quelques dizaines
d'euros d'erreur.
...
| Type | Présent en base |
|---|:---:|
| `BENEFITS_SAVEBACK` | oui |
| `STOCKPERK` | oui |
| `BONUS` | oui |
| `PEA_MARKETING` | oui |
| `GIFT` | oui (montant négatif) |

Soit, cumulés, plusieurs dizaines d'euros de résultat invisible. Il faut une catégorie « autres revenus/charges boursiers »,
...
**Vérifié en conditions réelles** sur les ETF détenus par l'utilisateur :
...
l'utilisateur sur les ETF réels :

- **Bug corrigé** : `JUSTETF_SECTOR_LABELS` avait été construite sur un seul fonds de reconnaissance
  et ne couvrait pas toute la taxonomie sectorielle réelle de justETF. Audit sur les ETF réels → 4
...
- **Confirmé, pas un bug** : le zonage géographique à 6 catégories (ex. Inde → "Marchés émergents")
  est voulu, cohérent avec `COUNTRY_TO_REGION` (couverture à 100 % vérifiée sur tous les ETF détenus). À la
...
- **Descriptions justETF** ajoutées pour les fonds (`MarketDataCache.description`, alimentée par
  `justetf_refresh`) — disponibles pour tous les ETF détenus, y compris ceux sans composition couverte
...
- **Vérifié en conditions réelles** : recette complète sur tous les ETF détenus (sectoriel corrigé, détail
  brut conforme à la fiche justETF réelle, description présente sur 26/26, prix cross-vérifiés
  contre l'API justETF en direct pour plusieurs ETF y compris ceux sans composition). Déclenchement
...
- **Description en français** : `fetch_composition` fait désormais une seconde requête, sur la page
  **française** de la fiche ETF (`/fr/etf-profile.html`), dédiée à la description et au top 10 — la
  ...
  libellés a été auditée sur les ETF réels et ne devait pas changer de langue source (les noms de
...
rafraîchissement complet (toutes les positions × plusieurs appels Yahoo) **de façon synchrone**. Le worker
...
déclenche une requête par position (autant de requêtes que de lignes pour afficher un tableau). Un `lazy="selectin"` ou
...
qui relit tout le grand livre et revalorise toutes les lignes… pour afficher deux pourcentages.
...
Des dizaines de lignes triées uniquement par ticker, sans colonne triable ni ligne de total. C'est le tableau le
...
> est aujourd'hui fausse pour plus de la moitié des ETF détenus, sans que rien ne l'indique — ce qui est pire
