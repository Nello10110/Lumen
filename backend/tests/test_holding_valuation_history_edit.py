...
`PATCH`/`DELETE /holdings/{holding_id}/immobilier-history/{point_id}` (adressé par
id, pas par ticker, depuis le 14/09/2026 : deux lignes peuvent désormais partager un
ticker, une par compte). Jusqu'ici, `enregistrer_point_historique` n'ajoutait qu'en
aveugle : une valeur tapée par erreur (ex. 0 €) restait figée pour toujours, sans
...
def test_modifier_un_point_qui_nest_pas_le_plus_recent_ne_touche_pas_la_valeur_courante(client, db):
    ...
    client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 200.0, "date": "2026-02-01"})
    point_id = _point_id(client, h.id, "2026-01-01")

    reponse = client.patch(f"/api/portfolio/holdings/{h.id}/immobilier-history/{point_id}", json={"valeur": 150.0, "date": "2026-01-01"})

    assert reponse.status_code == 200
    corps = reponse.json()
    assert corps["valeur_estimee"] == 200.0  # inchangée, le point de février reste le plus récent
    historique = client.get(f"/api/portfolio/holdings/{h.id}/immobilier-history").json()
...
def test_modifier_le_point_le_plus_recent_met_a_jour_la_valeur_courante(client, db):
    h = make_holding(db, ticker="AV1", type_actif="LIFE_INSURANCE")
    client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 100.0, "date": "2026-01-01"})
    client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 0.0, "date": "2026-02-01"})
    point_id = _point_id(client, h.id, "2026-02-01")

    reponse = client.patch(f"/api/portfolio/holdings/{h.id}/immobilier-history/{point_id}", json={"valeur": 250.0, "date": "2026-02-01"})
...
def test_avancer_la_date_dun_point_le_rend_le_plus_recent(client, db):
    ...
    reponse = client.patch(f"/api/portfolio/holdings/{h.id}/immobilier-history/{point_id}", json={"valeur": 300.0, "date": "2026-03-01"})
...
def test_supprimer_un_point_qui_nest_pas_le_plus_recent_ne_touche_pas_la_valeur_courante(client, db):
    ...
    reponse = client.delete(f"/api/portfolio/holdings/{h.id}/immobilier-history/{point_id}")
...
def test_supprimer_le_dernier_point_restant_vide_la_valeur_courante(client, db):
    ...
    assert client.get(f"/api/portfolio/holdings/{h.id}/immobilier-history").json() == []


def test_point_id_inexistant_renvoie_404(client, db):
    h = make_holding(db, ticker="AV1", type_actif="LIFE_INSURANCE")

    assert client.patch(f"/api/portfolio/holdings/{h.id}/immobilier-history/999999", json={"valeur": 1.0, "date": "2026-01-01"}).status_code == 404
    assert client.delete(f"/api/portfolio/holdings/{h.id}/immobilier-history/999999").status_code == 404
...
def test_valeur_negative_est_rejetee_a_la_modification(client, db):
    ...
    reponse = client.patch(f"/api/portfolio/holdings/{h.id}/immobilier-history/{point_id}", json={"valeur": -1.0, "date": "2026-01-01"})
...
def test_modifier_peut_ajouter_ou_corriger_le_versement_declare(client, db):
    ...
    h = make_holding(db, ticker="AV1", type_actif="LIFE_INSURANCE")
    client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 10500.0, "date": "2026-01-01"})
    point_id = _point_id(client, h.id, "2026-01-01")

    reponse = client.patch(
        f"/api/portfolio/holdings/{h.id}/immobilier-history/{point_id}", json={"valeur": 10500.0, "date": "2026-01-01", "versement": 500.0}
    )
...
def test_modifier_sans_versement_efface_un_versement_precedemment_declare(client, db):
    h = make_holding(db, ticker="AV1", type_actif="LIFE_INSURANCE")
    client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 10500.0, "date": "2026-01-01", "versement": 500.0})
    point_id = _point_id(client, h.id, "2026-01-01")

    client.patch(f"/api/portfolio/holdings/{h.id}/immobilier-history/{point_id}", json={"valeur": 10500.0, "date": "2026-01-01"})
