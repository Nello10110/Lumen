"""Point d'historique daté par l'utilisateur (backlog § 2.S.1, écran Épargne) —
`PUT /holdings/{holding_id}/valorisation` (adressé par id, pas par ticker, depuis le
14/09/2026 : deux lignes peuvent désormais partager un ticker, une par compte), seule
route qui accepte une date choisie par le client plutôt que `datetime.now()` (cf.
`create_holding`/`update_holding`, inchangés). Règle d'antidatage : un point antidaté
ne doit jamais écraser une valeur courante plus récente déjà connue."""

from .conftest import ID_UTILISATEUR_B, ID_UTILISATEUR_TEST, NOM_UTILISATEUR_B, NOM_UTILISATEUR_TEST, basculer_utilisateur, make_holding


def test_ajoute_un_point_dhistorique_a_la_date_choisie(client, db):
    h = make_holding(db, ticker="AV1", type_actif="LIFE_INSURANCE")

    reponse = client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 10000.0, "date": "2026-03-15"})

    assert reponse.status_code == 200
    historique = client.get(f"/api/portfolio/holdings/{h.id}/immobilier-history").json()
    assert len(historique) == 1
    assert historique[0]["valeur"] == 10000.0
    assert historique[0]["date_valeur"].startswith("2026-03-15")


def test_versement_declare_est_conserve_sur_le_point(client, db):
    """Backlog § U.2 (retour utilisateur 30/08/2026) : le foyer peut préciser la part
    de la hausse qui vient d'un versement plutôt que d'une performance du contrat."""
    h = make_holding(db, ticker="AV1", type_actif="LIFE_INSURANCE")

    reponse = client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 10500.0, "date": "2026-03-15", "versement": 500.0})

    assert reponse.status_code == 200
    historique = client.get(f"/api/portfolio/holdings/{h.id}/immobilier-history").json()
    assert historique[0]["versement"] == 500.0


def test_versement_absent_reste_none(client, db):
    h = make_holding(db, ticker="AV1", type_actif="LIFE_INSURANCE")

    client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 10000.0, "date": "2026-03-15"})

    historique = client.get(f"/api/portfolio/holdings/{h.id}/immobilier-history").json()
    assert historique[0]["versement"] is None


def test_met_a_jour_la_valeur_courante_quand_le_point_est_le_plus_recent(client, db):
    h = make_holding(db, ticker="AV1", type_actif="LIFE_INSURANCE")
    client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 10000.0, "date": "2026-01-01"})

    reponse = client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 10500.0, "date": "2026-02-01"})

    assert reponse.status_code == 200
    corps = reponse.json()
    assert corps["valeur_estimee"] == 10500.0
    assert corps["date_valeur_estimee"].startswith("2026-02-01")


def test_un_point_antidate_najamais_ecraser_une_valeur_plus_recente(client, db):
    """Cas verrouillé explicitement (backlog 2.S.1) : un rattrapage a posteriori
    (saisie tardive d'un mois passé) ne doit jamais corrompre la valeur "courante"
    déjà connue plus récente."""
    h = make_holding(db, ticker="AV1", type_actif="LIFE_INSURANCE")
    client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 10500.0, "date": "2026-03-01"})

    reponse = client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 9800.0, "date": "2026-01-15"})

    assert reponse.status_code == 200
    corps = reponse.json()
    # La valeur courante reste celle de mars, pas écrasée par le point de janvier.
    assert corps["valeur_estimee"] == 10500.0
    assert corps["date_valeur_estimee"].startswith("2026-03-01")
    # Mais le point antidaté est bien conservé dans l'historique complet, pas perdu.
    historique = client.get(f"/api/portfolio/holdings/{h.id}/immobilier-history").json()
    valeurs = sorted(p["valeur"] for p in historique)
    assert valeurs == [9800.0, 10500.0]


def test_valorisation_sur_holding_introuvable_renvoie_404(client):
    reponse = client.put("/api/portfolio/holdings/999999/valorisation", json={"valeur": 100.0, "date": "2026-01-01"})

    assert reponse.status_code == 404


def test_valorisation_sur_actif_dun_autre_utilisateur_est_refusee(client, db):
    basculer_utilisateur(db, ID_UTILISATEUR_B, NOM_UTILISATEUR_B)
    h_b = make_holding(db, ticker="AV_B", user_id=ID_UTILISATEUR_B, type_actif="LIFE_INSURANCE")
    basculer_utilisateur(db, ID_UTILISATEUR_TEST, NOM_UTILISATEUR_TEST)

    reponse = client.put(f"/api/portfolio/holdings/{h_b.id}/valorisation", json={"valeur": 100.0, "date": "2026-01-01"})

    assert reponse.status_code == 404


def test_valeur_negative_est_rejetee(client, db):
    h = make_holding(db, ticker="AV1", type_actif="LIFE_INSURANCE")

    reponse = client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": -100.0, "date": "2026-01-01"})

    assert reponse.status_code == 400


def test_date_mal_formee_est_rejetee(client, db):
    h = make_holding(db, ticker="AV1", type_actif="LIFE_INSURANCE")

    reponse = client.put(f"/api/portfolio/holdings/{h.id}/valorisation", json={"valeur": 100.0, "date": "15/01/2026"})

    assert reponse.status_code == 400


def test_creation_et_edition_classiques_gardent_datetime_now_inchange(client):
    """Non-régression : seule la nouvelle route `valorisation` accepte une date
    choisie — `create_holding`/`update_holding` continuent de stamper `datetime.now()`
    pour tout appelant qui ne passe pas par elle (comportement déjà couvert par
    `test_holding_immobilier.py`, revérifié ici pour un type Épargne)."""
    cree = client.post(
        "/api/portfolio/holdings",
        json={"ticker": "AV1", "quantite": 1, "type_actif": "LIFE_INSURANCE", "valeur_estimee": 5000.0, "compte_nom": "Assurance-vie"},
    ).json()

    assert cree["date_valeur_estimee"] is not None
    historique = client.get(f"/api/portfolio/holdings/{cree['id']}/immobilier-history").json()
    assert len(historique) == 1
    assert historique[0]["valeur"] == 5000.0
