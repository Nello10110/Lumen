...
# --- `PUT /holdings/{holding_id}/immobilier` ---------------------------------------
# Adressé par `holding_id` (revu le 14/09/2026, pas par ticker) : deux lignes peuvent
# désormais partager un ticker (une par compte) — cf. `routers/portfolio.py`.


def test_creer_le_detail_immobilier_dune_ligne(client, db):
    holding = make_holding(db, ticker="MAISON", type_actif="REAL_ESTATE", prix_revient_moyen=200000.0, valeur_estimee=250000.0)

    reponse = client.put(f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier())
...
def test_mettre_a_jour_le_detail_immobilier_remplace_les_valeurs(client, db):
    ...
    reponse = client.put(f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier(loyer_mensuel=1100.0))

    assert reponse.status_code == 200
    assert reponse.json()["loyer_mensuel"] == 1100.0


def test_immobilier_sur_holding_introuvable_renvoie_404(client):
    reponse = client.put("/api/portfolio/holdings/999999/immobilier", json=_payload_immobilier())
...
def test_loyer_negatif_est_rejete(client, db):
    ...
    reponse = client.put(f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier(loyer_mensuel=-100.0))
...
def test_surface_nulle_est_rejetee(client, db):
    ...
    reponse = client.put(f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier(surface_m2=0))

    assert reponse.status_code == 400


# --- Cashflow / rentabilité, via `GET /holdings/{holding_id}/detail` ---------------


def test_detail_dune_ligne_sans_fiche_immobiliere_renvoie_immobilier_null(client, db):
    holding = make_holding(db, ticker="AAPL", type_actif="STOCK")
...
def test_frais_acquisition_augmente_le_prix_dacquisition_total_et_baisse_la_rentabilite(client, db):
    ...
    reponse = client.put(
        f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier(frais_notaire=10000.0, frais_travaux=5000.0)
    )
    corps = reponse.json()
...
def test_frais_notaire_negatif_est_rejete(client, db):
    ...
    reponse = client.put(f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier(frais_notaire=-500.0))
...
def test_frais_travaux_negatif_est_rejete(client, db):
    ...
    reponse = client.put(f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier(frais_travaux=-500.0))
...
def test_frais_acquisition_autres_negatif_est_rejete(client, db):
    ...
    reponse = client.put(
        f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier(frais_acquisition_autres=-500.0)
    )
...
def test_simulation_loyer_estime_negatif_est_rejete(client, db):
    ...
    reponse = client.put(
        f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier(simulation_loyer_estime=-100.0)
    )
...
def test_simulation_taxe_habitation_negative_est_rejetee(client, db):
    ...
    reponse = client.put(
        f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier(simulation_taxe_habitation_annuelle=-100.0)
    )
...
def test_residence_principale_par_defaut_a_false_et_peut_etre_activee(client, db):
    ...
    corps_par_defaut = client.put(f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier()).json()
    assert corps_par_defaut["residence_principale"] is False

    corps_active = client.put(
        f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier(residence_principale=True)
    ).json()
...
def test_sans_loyer_seul_prix_m2_est_calcule(client, db):
    ...
    holding = make_holding(db, ticker="MAISON", type_actif="REAL_ESTATE", prix_revient_moyen=200000.0, valeur_estimee=250000.0)
    client.put(f"/api/portfolio/holdings/{holding.id}/immobilier", json=_payload_immobilier(loyer_mensuel=None))
...
# --- Historique de valorisation (`GET /holdings/{holding_id}/immobilier-history`) --
...
def test_creer_une_ligne_avec_valeur_estimee_ajoute_un_point_dhistorique(client):
    ...
    holding_id = reponse.json()["id"]

    historique = client.get(f"/api/portfolio/holdings/{holding_id}/immobilier-history").json()
...
def test_mettre_a_jour_valeur_estimee_ajoute_un_point_sans_effacer_le_precedent(client):
    ...
    historique = client.get(f"/api/portfolio/holdings/{cree['id']}/immobilier-history").json()
...
def test_ligne_sans_valeur_estimee_na_aucun_point_dhistorique(client, db):
    ...
    historique = client.get(f"/api/portfolio/holdings/{holding.id}/immobilier-history").json()
